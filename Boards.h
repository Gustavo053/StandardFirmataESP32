...
// ESP32: by Josenalde Oliveira 26.11.2019

// note: at boot mode: can be used as outputs/inputs (except 0,1,3)

// GPIOs 0,5,14,15 (pwm output), 1 (debug output), 3 (high), 12 (low)

// GPIOs 6-11 are connected to the integrated SPI flash

// ADC: 08 ADC1_CH0...CH7 and 10 ADC2_CH0..CH9 (do not use ADC2 and Wifi)
...
#define IS_PIN_SPI(p)           ((p) == CS || (p) == MOSI || (p) == MISO || (p) == CLK) // or numbers?
#define IS_PIN_INTERRUPT(p)     (digitalPinToInterrupt(p) > NOT_AN_INTERRUPT)
#define IS_PIN_SERIAL(p)        ((p) == PIN_SERIAL_RX || (p) == PIN_SERIAL_TX)
#define PIN_TO_DIGITAL(p)       (p)
...
static inline unsigned char writePort(byte port, byte value, byte bitmask)
{
  ...
  if ((bitmask & 0x80)) digitalWrite(PIN_TO_DIGITAL(pin + 7), (value & 0x80)/*HIGH*/);
  return 1;
#endif
}
