...
void setPinModeCallback(byte pin, int mode)
{
  ...
  case PIN_MODE_ANALOG:
    if (IS_PIN_ANALOG(pin))
    {
      //**Edition ==> lines added by Gustavo Pereira, team ind4fibre
      if (mode == PIN_MODE_ANALOG)
      {
        reportAnalogCallback(PIN_TO_ANALOG(pin), 1 /*OUTPUT*/);
      }
      //**
      if (IS_PIN_DIGITAL(pin))
      {
  ...
  case PIN_MODE_OUTPUT:
    ...
    break;
  case PIN_MODE_PWM:
    if (IS_PIN_PWM(pin) && mode == PIN_MODE_PWM)
    {
      pinMode(PIN_TO_PWM(pin), OUTPUT);
      /* changed by ind4fibre team 11.27.2019 for ESP32 */
#if defined(ARDUINO_ARCH_ESP32)
      ledcAttachPin(PIN_TO_PWM(pin), pin - 15); //set channel for esp32 adc
      ledcSetup(pin - 15, 5000, 8);             //set channel for esp32 adc, freq and pwm 8 bits
      ledcWrite(pin - 15, 0);                   //channel 0, value 0
#else
      pinMode(PIN_TO_PWM(pin), OUTPUT);
      analogWrite(PIN_TO_PWM(pin), 0);
#endif
      /* previous version standard firmata for ARDUINO
        pinMode(PIN_TO_PWM(pin), OUTPUT);
        analogWrite(PIN_TO_PWM(pin), 0);
        */
...
void reportAnalogCallback(byte analogPin, int value)
{
  if (analogPin < TOTAL_ANALOG_PINS)
  {
    ...
    {
      byte pino[TOTAL_ANALOG_PINS];
      for (int i = 0, j = 32; i < TOTAL_ANALOG_PINS, j <= 32 + TOTAL_ANALOG_PINS + 1; i++, j++)
      {
        pino[i] = j;
      }
      analogInputsToReport = analogInputsToReport | (1 << analogPin);
      ...
      if (!isResetting)
      {
        ...
        // reconnecting.
        Firmata.sendAnalog(analogPin, analogRead(/*analogPinpino*/ pino[analogPin])); //Edition
      }
    }
  }
  // TODO: save status to EEPROM here, if changed
...
void sysexCallback(byte command, byte argc, byte *argv)
{
  ...
  case 0x02: //read and write digital
    ...
    pinReadDigital = argv[0];
    pinWriteDigital = argv[1];

    pinMode(pinReadDigital, INPUT);
    pinMode(pinWriteDigital, OUTPUT);
  ...
  case 0x03: //read and write analog
    ...
    while (analogRead(pinReadAnalog))
    {
      ...
      ledcWrite(pinWriteAnalog, analogValue);
    }
    Firmata.sendSysex(command, argc, argv); // callback
    break;
  ...
  case CAPABILITY_QUERY:
    ...
    for (byte pin = 0; pin < TOTAL_PINS; pin++)
    {
      ...
      if (IS_PIN_ANALOG(pin))
      {
        ...
        Firmata.write(10); // default ADC ESP32 resolution
